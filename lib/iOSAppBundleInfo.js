...
(function() {
  ...
  iOSAppBundleInfo = (function(superClass) {
    ...
    iOSAppBundleInfo.prototype.getIconFile = function(callback) {
      var _plist, createSubNames, find, lookupType, self;
      createSubNames = function(initial, endname) {
        return [initial + '@3x' + endname + '.png', initial + '@2x' + endname + '.png'];
      };
      find = function(index, lookup, cb) {
        if (!lookup[index]) {
      ...
      lookupType = function(origin, lookup, cb) {
      ...
      lookupType(_plist.CFBundleIcons, [], function(err, datas) {
    	...
    	// Try above method first....
    iOSAppBundleInfo.prototype.getCustomIconFile = function(pattern, callback) {
      return this.findFileStream('Payload/*.app/' + pattern, function(err, stream) {
        if (err) {
          return callback(err);
        }
        if (!stream) {
